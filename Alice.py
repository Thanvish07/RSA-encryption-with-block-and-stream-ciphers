...
class AliceGUI:
    ...
    def load_keys(self):
        ...
        try:
            ...
            print(f"Error loading Alice's private key: {e}")
        ...
        try:
            ...
            print(f"Error loading Bob's public key: {e}")
    ...
    def encrypt(self):
        ...
        if encryption_type == "Block Cipher":
            ...
            # Encode tag ('A' for Alice) + key, IV, and ciphertext for transmission
            tag = b'A'  # Tag to identify Alice's encryption
            ...
            # Encode tag ('A' for Alice) + key and nonce for transmission
            tag = b'A'  # Tag to identify Alice's encryption
    ...
    def decrypt(self):
        ...
        try:
            ...
            if tag != b'B':  # Ensure it is a ciphertext from Bob
                raise ValueError("Decryption Error: Ciphertext not encrypted by Bob.")
...
def start_client():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        root = tk.Tk()
        alice_gui = AliceGUI(root, sock)

        # Start a separate thread to listen for incoming messages
        threading.Thread(target=alice_gui.receive, daemon=True).start()
